...
export class ZagsBirthRecordsService {
  ...
  create(payload: CreateZagsBirthRecordRequest): Observable<boolean> {
    return this.http.post(this.apiUrl, payload, { observe: 'response' }).pipe(map((response) => response.ok));
  }

  update(id: number, payload: CreateZagsBirthRecordRequest): Observable<boolean> {
    return this.http.put(`${this.apiUrl}/${id}`, payload, { observe: 'response' }).pipe(map((response) => response.ok));
  }
